...
class BrokerFabrica {
    ...
    static ISipsaBroker getBroker(Class c) throws SipsaExcepcion {
        ...
        if (c.equals(Fabrica.class)) {
            broker = new FabricaBroker();
        }
        if (c.equals(Modelo.class)) {
            broker = new ModeloBroker();
        }
        if (c.equals(OrdenDeTrabajo.class)) {
            broker = new OtBroker();
        }
        if (c.equals(Pac.class)) {
            broker = new PacBroker();
        }
        if (c.equals(Producto.class)) {
            broker = new ProductoBroker();
        }
        if (c.equals(Pv.class)) {
            broker = new PvBroker();
        }
        if (c.equals(TipoProducto.class)) {
            broker = new TipoProductoBroker();
        }
        if (c.equals(Venta.class)) {
            broker = new VentaBroker();
        }
        if (broker == null) {
